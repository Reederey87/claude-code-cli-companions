...
test("Grok commands expose rescue forwarding and session hooks", () => {
  ...
  assert.match(rescue, /default to foreground for read-only tasks and to background for `--write` tasks/i);
  assert.match(rescue, /status incomplete/i);
  ...
  assert.match(agent, /prefer background execution \(foreground write jobs are killed with a misleading Cancelled stop/i);
  assert.match(agent, /do not silently background it/i);
